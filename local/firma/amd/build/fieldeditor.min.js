define([], function() {
    ...
    const init = () => {
        ...
        if (!configData) {
            console.error('local_firma/fieldeditor: no editor configuration found');
            return;
        }
        ...
        try {
            ...
            console.error('local_firma/fieldeditor: failed to parse editor configuration', e);
            return;
        }
            ...
            fields.forEach((field) => {
                ...
                row.innerHTML = `
                    <div class="local-firma-field-row__header">
                        ...
                        <button type="button" class="btn btn-icon btn-link field-delete" aria-label="${strings.deleteField || 'Eliminar'}">×</button>
                    </div>
                    <div class="local-firma-field-row__body">
                        <label><span>${escapeHtml(getLabel('fieldSource'))}</span>
                            <select class="custom-select field-source">${getSourceOptions(field.source)}</select>
                        </label>
                        <label><span>${escapeHtml(getLabel('fieldPage'))}</span>
                            <input type="number" class="form-control field-page" value="${field.page}" min="1">
                        </label>
                        <label><span>${escapeHtml(getLabel('fieldX'))}</span>
                            <input type="number" class="form-control field-x" step="0.5" value="${field.x}">
                        </label>
                        <label><span>${escapeHtml(getLabel('fieldY'))}</span>
                            <input type="number" class="form-control field-y" step="0.5" value="${field.y}">
                        </label>
                        <label><span>${escapeHtml(getLabel('fieldFont'))}</span>
                            <input type="number" class="form-control field-fontsize" step="0.5" value="${field.fontsize}">
                        </label>
                        <label><span>${escapeHtml(getLabel('fieldWidth'))}</span>
                            <input type="number" class="form-control field-width" step="0.5" value="${field.width}">
                        </label>
                        <label><span>${escapeHtml(getLabel('fieldValue'))}</span>
        ...
        const createBox = (field) => {
            ...
            box.textContent = field.label || (config.fieldOptions && config.fieldOptions[field.source]) || field.source;
        ...
        const prepareDocument = () => {
            if (!hasPdfJs) {
                // Sin PDF.js: mostrar un fondo gris con mensaje y dejar que funcionen los campos
                ...
                ctx.fillText('(PDF.js no cargado)', 297, 445);
        ...
        pageSelect.addEventListener('change', () => {
            currentPage = parseInt(pageSelect.value, 10) || 1;
            if (hasPdfJs) { renderPage(); } else { renderBoxes(); }
        });

        renderFieldList();
        if (fields.length) { setActive(fields[0].id); }
        prepareDocument();
    };
